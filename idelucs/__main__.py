...
def run(args):
    ...
    save_results_in_file(dataset_name, "iDeLUCS", args, results, f'{hour}:{minutes}:{seconds}', memory, f'{os.getcwd()}/ALL_RESULTS.tsv') #os.getcwd())
    ...
    if args['plot']:
        ...
        ax.scatter(embedding[mask & clustered, 0],
                  embedding[mask & clustered, 1],
                  c=y_pred[mask & clustered],
                  s=1,
                  cmap='Spectral')
        fig.savefig(f'{results_folder}/{time_stamp}/learned_representation.jpg')
...
def main():
    ...
    parser.add_argument('--plot', action='store',type=bool, default=False)
