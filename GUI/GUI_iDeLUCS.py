...
from idelucs.utils import SummaryFasta, plot_confusion_matrix, \
                      label_features, compute_results, PlotPolygon
...
class ResultsThread(QThread):
    ...
    def run(self):
        ...
        if len(self.model.predictions) <= 1:
            ...
            if self.model.n_clusters == 200:
                clusterer = hdbscan.HDBSCAN(min_cluster_size=len(self.model.names)//100 + 1, gen_min_span_tree=True, prediction_data=True)
                clusterer.fit(self.model.latent)
                y_pred = clusterer.labels_ + 1
                probabilities = clusterer.probabilities_
                self.model.n_clusters = np.max(y_pred) + 1
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.input_n_clusters.setMinimum(0)
        self.input_n_clusters.setMaximum(199)
        ...
        self.Summary.setGeometry(QtCore.QRect(46,  305, 190,  34)) #46,  15, 174,  34
        ...
        self.Parameter_Summary.setGeometry(QtCore.QRect(46, 13, 190, 34))
        ...
        self.progress_info.setGeometry(QtCore.QRect(425, 380, 300,  27)) #366, 400, 180,  27
        font = QtGui.QFont()
        font.setPointSize(10)
    ...
    def evt_training_finished(self):
        ...
        if self.args['n_clusters'] < 200:
            self.progress_info.setText("Preparing results ...")
        else:
            self.progress_info.setText("Calling HDBSCAN, this might take a while ...")
...
if __name__ == "__main__":
    ...
    # set stylesheet
    file = QFile("theme/light/stylesheet.qss")
